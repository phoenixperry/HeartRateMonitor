...
/// A breathing hexagon view that animates at the speed of a given BPM and morphs toward circle based on sync level.
/// Each BPM cycle follows a sine waveform from 0 to 2π.
/// Triggers `onCycleComplete` at the end of each beat cycle.
struct WaveformBreathingHexagon: View {
    ...
    var body: some View {
        ...
        if shouldAnimate && bpm > 0 {
            TimelineView(.animation) { _ in
                ...
                // The animated morphing hexagon
                MorphingHexagonToCircle(
            ...
            .onAppear {
                ...
                lastCycleTime = Date()
                print("🔴 Hexagon animation started with BPM: \(currentBPM)")
            }
            ...
            // Static shape when not animating
            MorphingHexagonToCircle(
                morphLevel: morphLevel,
                color: bpm > 0 ? color.opacity(0.5) : Color.gray.opacity(0.3)
            )
        }
    }
